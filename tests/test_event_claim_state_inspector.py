...
class EventClaimStateInspectorTests(unittest.TestCase):
    ...
    def test_inspector_is_bounded_and_passive(self):
        ...
        self.assertIn("structuralQuestMatch", self.source)
        self.assertIn("type(value.Accepted)", self.source)
        self.assertIn("value.canClaim ~= nil", self.source)
        self.assertNotIn("shallowMatch", self.source)
